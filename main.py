...
WORKER_VERSION = "v10g-weld-once-2026-06-19"
...
3) НАЗВАНИЯ сортамента (как в «Сводной»):
   ...
   - КРУГ (арматура): знак диаметра перед числом ОЗНАЧАЕТ КРУГ. Знак диаметра
     может быть отрисован по-разному: «⌀», «Ø», «∅», «Ç», «C» с чертой, «q», «d»,
     «d=», «Ç20», «Ç16». ВСЕ они = круг. Пример: «Ç20» → "Круг 20"; «Ç16» → "Круг 16";
     «q20» → "Круг 20". ВСЕГДА пиши именно "Круг <число>".
     • ЗАПРЕЩЕНО подставлять «Катанка» вместо круга. «Катанка 6,5»/«Катанка 8» —
       это ОТДЕЛЬНЫЙ материал (проволока-заготовка), НЕ результат знака диаметра.
       Никогда не превращай «Ç20»/«⌀20» в «Катанку».
     • ЗАПРЕЩЕНО подставлять «Труба» вместо круга.
     • НЕ теряй ни одной такой позиции: и «Ç20», и «Ç16» должны попасть в ответ
       как "Круг 20" и "Круг 16" (с просуммированной массой по диаметру).
...
6) control_total_kg = масса элемента, КАК УКАЗАНА в итоговой строке спецификации
   «Масса элемента» (напр. 108.96). НЕ вычитай сварку сам — верни как на чертеже.
   Вычитание сварки сделает программа по полю weld_pct. Если итоговой массы нет — null.
