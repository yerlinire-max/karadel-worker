...
- sortament: тип профиля + размер. Примеры из таких чертежей:
  "Швеллер 10П", "Уголок L100x10", "Уголок L75x5", "Двутавр 30Б1",
  "Лист t10", "Труба квадратная 100х100х5".
...
- НЕ повторяй одну и ту же позицию. Каждый профиль — один раз; если он встречается
  в разных таблицах, просуммируй массу.
...
def build_raschet_cells(positions, drawing_name):
    ...
    agg, order = {}, []
    for p in positions:
        name = match_sortament(p.get("sortament") or p.get("name") or "")
        if not name:
            continue
    ...
    for i, name in enumerate(order):
        ...
        c = STEEL_COLUMNS[i]
        cells[f"{c}5"] = (name, "s")
        cells[f"{c}6"] = (round(agg[name], 2), "n")
        total += agg[name]
